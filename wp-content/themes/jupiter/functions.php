...
// Custom : WP Shortcode
function sc_monzon_wp_search(){
echo '<div class="vc_wp_search wpb_content_element"><div class="widget widget_search" style="float:right;width:85%">
<form class="mk-searchform" method="get" id="searchform" action="http://monzon8.es/wordpress-qah">
	<input value="Buscar" type="submit" class="mk-button" type="submit" style="height:40px; background-color:#ff4a69; color:#FFF; padding:0 12px; float:right"/>
	<input type="text" class="text-input" placeholder="¿Qué quieres aprender hoy?" value="" name="s" id="s" style="width:80% !important; float:right; text-align:center; color:rgb(0, 113, 255); font-size:20px; height:40px"/>
</form></div></div>';
}
add_shortcode('monzon_wp_search', 'sc_monzon_wp_search');

// Custom : WP Shortcode
function sc_monzon_grid($atts = array()){
...
require("monzon-grid.php");
   

}
add_shortcode('monzon_grid', 'sc_monzon_grid');


// Custom : WP Shortcode
function sc_monzon_grid_ranking($atts = array()){
...
require("monzon-grid-ranking.php");
   

}


add_shortcode('monzon_grid_ranking', 'sc_monzon_grid_ranking');

function monzon_scripts_styles() {

wp_enqueue_style( 'custom-style', get_template_directory_uri() . '/monzon.css' ); //our stylesheet


}
add_action( 'wp_enqueue_scripts', 'monzon_scripts_styles' );
